  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {session && <nav aria-label="Navegação principal" className="fixed inset-x-3 bottom-3 z-50 mx-auto flex max-w-lg items-center justify-around rounded-2xl border border-white/12 bg-[#090b1c]/84 px-1 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,.07),0_14px_32px_rgba(0,0,0,.36)] backdrop-blur-xl">
        {navLinks.map(({ href, label, icon: Icon, featured }) => <Link key={href} href={href} className={cn("flex min-w-0 cursor-pointer flex-col items-center gap-1 rounded-xl px-2 py-2 text-[9px] transition-all sm:min-w-15 sm:px-3 sm:text-[10px]", featured && 'relative -mt-5 min-w-14 rounded-2xl border border-primary/30 bg-primary/18 px-3 py-3 text-primary shadow-[0_10px_24px_rgba(201,168,76,.2)]', location === href ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-white/5 hover:text-foreground")}><Icon className={cn('h-4 w-4', featured && 'h-5 w-5')} />{label}</Link>)}
      </nav>}
