...
import { Camera, Check, LoaderCircle, LogOut, Save, UserRound, X, ZoomIn, ZoomOut } from 'lucide-react';
...
export default function Profile() {
  ...
  return <div className="panel-enter mx-auto max-w-xl space-y-7 pb-24">
    ...
    {confirmLogout && <div className="fixed inset-0 z-[95] grid place-items-center bg-black/75 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-sm rounded-3xl border border-white/15 bg-[#101126] p-6 shadow-2xl"><h2 className="font-display text-xl">Sair da conta?</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Seu perfil continua salvo. Para voltar, use seu nome e senha.</p><div className="mt-6 flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => setConfirmLogout(false)}>Cancelar</Button><Button type="button" onClick={() => { clearSession(); setLocation('/'); }} className="border border-red-300/30 bg-red-500/20 text-red-100 hover:bg-red-500/30">Sair</Button></div></div></div>}
